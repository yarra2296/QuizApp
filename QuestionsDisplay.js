...
export default class QuestionsDisplay extends React.Component{
    ...
    ExitExam(){
        this.props.navigation.state.params.onSubmit(this.state.answersArray);
    ...
    render(){
        ...
        return(
            <View style={{flex: 1, backgroundColor: '#ffffff', paddingTop: 5}}>
               ...
               {/* <TimerCountdown
                    initialSecondsRemaining={600000}
                    onTick={secondsRemaining => console.log('tick', secondsRemaining)}
                    onTimeElapsed={() => navigate('Answers', {value: this.state.answersArray, data: params.data})}
                    allowFontScaling={true}
                    style={{ fontSize: 20 }}
                />*/}
                        <CountdownCircle
                            seconds={(params.data.duration)*60}
                            radius={30}
                            borderWidth={8}
                            color="#ff003f"
                            bgColor="#fff"
                ...
                <ScrollView contentContainerStyle={{flexGrow:1}}>
                        <View style={{marginTop: 10}}>
                            ...
                            }}>{this.state.questionNumber+1+') '+  params.data.questions[this.state.questionNumber].question}</Text>
                            <View style={{marginBottom: 10, flex: 1}}>
                                {params.data.questions[this.state.questionNumber].options.map((opt, key) =>
                ...
                <View style={{flex: 1,justifyContent: 'flex-end', bottom: 100, margin: 10}}>
                    <View style={{flexDirection:'row', justifyContent: 'space-between'}}>
                        ...
                        {this.state.questionNumber === questionsLength ? <TouchableOpacity onPress={() => navigate('Answers', {value: this.state.answersArray, data: params.data}, this.setState({Screen: true}))} style={{backgroundColor: 'green', borderRadius: 5}}>
                ...
                <View style={{bottom: 70}}>
                    ...
                    <View>
                        <TouchableOpacity onPress={()=>this.ExitExam()} style={{backgroundColor: 'blue', borderRadius: 5}}>
                            <Text style={{color: '#ff1a15', fontSize: 15, padding: 20, left : 140}}>Exit Exam</Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView>
            </View>
